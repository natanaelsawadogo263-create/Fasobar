...
type SessionsPeriodFilter = "day" | "week" | "month" | "custom";
...
type AdminBarSessionsWorkspaceProps = {
  ...
  periodFilter?: SessionsPeriodFilter;
  periodFrom?: string;
  periodTo?: string;
};

const PERIOD_OPTIONS: Array<{ id: Exclude<SessionsPeriodFilter, "custom">; label: string }> = [
  { id: "day", label: "Jour" },
...
export function AdminBarSessionsWorkspace({
  ...
  function applyPeriodFilters(next: {
    ...
    router.push(`/application/sessions-bar?${params.toString()}`);
  }
        ...
        {sessions.length === 0 ? (
          <div className="flex h-full flex-col items-center justify-center px-4 py-16 text-center">
            ...
            <p className="mt-1 max-w-sm text-[12px] text-slate-500">
              Les sessions du bar pour cette période apparaîtront ici.
            </p>
          </div>
        ) : (
