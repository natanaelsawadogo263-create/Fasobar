-- FasoBar: catalogue caisse (catégories + produits maquette POS)
...
BEGIN
  ...
  IF NOT public.user_can_manage_orders(v_user_id, p_establishment_id) THEN
...
REVOKE ALL ON FUNCTION public.ensure_caisse_catalog(uuid) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.ensure_caisse_catalog(uuid) TO authenticated;

-- Backfill catégories pour tous les établissements existants
...
CROSS JOIN (
  VALUES
    ...
    ('Accompagnements', 'accompagnements', 'KITCHEN'::public.department_code),
    ('Desserts', 'desserts', 'KITCHEN'::public.department_code)
) AS seed(name, slug, dept_code)
INNER JOIN public.departments d
  ON d.establishment_id = e.id
 AND d.code = seed.dept_code
ON CONFLICT (establishment_id, slug) DO UPDATE
  SET name = EXCLUDED.name,
      active = true,
      updated_at = now();

-- Backfill produits caisse pour tous les établissements existants
INSERT INTO public.products (
  organization_id,
  establishment_id,
  department_id,
...
CROSS JOIN (
  VALUES
    ...
    ('FRITE', 'frite', 'KITCHEN'::public.department_code, 'Accompagnements', 500, 'PORTION'::public.product_unit),
    ('TIRAMISU', 'tiramisu', 'KITCHEN'::public.department_code, 'Desserts', 800, 'PORTION'::public.product_unit)
) AS seed(name, slug, dept_code, category_name, selling_price, unit)
INNER JOIN public.departments d
  ON d.establishment_id = e.id
 AND d.code = seed.dept_code
