-- FasoBar: seuls les rôles caissier (CASHIER_KITCHEN / CASHIER / KITCHEN_MANAGER)
-- peuvent ouvrir ou fermer une session de caisse. L'Admin reste en lecture seule.
...
AS $$
  SELECT EXISTS (
    ...
    WHERE em.user_id = p_user_id
      ...
      AND em.role IN ('CASHIER_KITCHEN', 'CASHIER', 'KITCHEN_MANAGER')
  )
  AND NOT EXISTS (
    SELECT 1
...
AS $$
  SELECT EXISTS (
    ...
    WHERE s.id = p_session_id
      ...
      AND public.user_is_cash_register_operator(p_user_id, s.establishment_id)
  );
$$;
...
REVOKE ALL ON FUNCTION public.user_is_cash_register_operator(uuid, uuid) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.user_is_cash_register_operator(uuid, uuid) TO authenticated;
