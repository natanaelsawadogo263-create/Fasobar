-- FasoBar: sync desktop — si la commande existe déjà, mettre à jour les articles
-- et renvoyer bar/cuisine en TO_PREPARE quand des boissons/plats sont ajoutés.
-- Dépend de 20260811120000_reopen_order_reset_bar_kitchen_prep.sql
...
BEGIN
  ...
  -------------------------------------------------------------------------
  -- ORDER_CREATED (création OU mise à jour si la commande existe déjà)
  -------------------------------------------------------------------------
  IF p_event_type = 'ORDER_CREATED' THEN
    ...
    -- Commande déjà cloud (réouverture caisse / ajouts) → remplacer articles + prep
    IF EXISTS (SELECT 1 FROM public.orders o WHERE o.id = v_order_id) THEN
      PERFORM public.desktop_replace_order_items_and_prep(
        v_order_id,
        v_organization_id,
        v_establishment_id,
    ...
    -- Première sync : initialiser bar/cuisine à préparer si articles présents
...
COMMENT ON FUNCTION public.apply_desktop_outbox_event(text, text, jsonb) IS
  'Ingest idempotent Desktop. ORDER_CREATED crée ou met à jour les articles et le statut bar/cuisine (TO_PREPARE si ajouts).';
